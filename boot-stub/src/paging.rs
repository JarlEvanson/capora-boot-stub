...
use crate::mapper::{ApplicationMemoryMap, VirtualMemoryMap};
...
/// Creates and allocates a full page table layout for the provided [`VirtualMemoryMap`], returning
/// the physical memory address to be loaded into the address space specifier.
pub fn map(memory_map: VirtualMemoryMap) -> u64 {
    let mut pml4e_index = 512;
    let mut pml3e_index = 512;
    let mut pml2e_index = 512;
    ...
    for entry in memory_map.as_slice().iter() {
        let mut page = entry.page();
        while entry.virt_contains(page << 12) {
            let pml4e = get_pml4e_page_index(page);
            if pml4e != pml4e_index {
                pml4e_index = pml4e;
                page_count += 1;
